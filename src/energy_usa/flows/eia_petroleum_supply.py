...
EIA_PETROLEUM_SUPPLY_COLUMNS = ["value"]
EIA_PETROLEUM_SUPPLY_PATH = "petroleum/sum/snd/data"


@task(name="fetch-eia-petroleum-supply")
async def fetch_eia_petroleum_supply(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_PETROLEUM_SUPPLY_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_PETROLEUM_SUPPLY_PATH, **params)
...
@task(name="upsert-petroleum-supply")
def upsert_petroleum_supply_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_petroleum_supply(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-petroleum-supply", retries=2)
async def ingest_eia_petroleum_supply(
    ...
    data = await fetch_eia_petroleum_supply(
    ...
    total = upsert_petroleum_supply_task(settings.effective_ingest_url, data)
    if total == 0:
