"""Prefect flow: fetch EIA densified-biomass/capacity-by-region and upsert.

Annual wood pellet capacity by region. Period stored as DATE (Jan 1).
"""
...
EIA_BIOMASS_CAPACITY_COLUMNS = ["capacity"]
EIA_BIOMASS_CAPACITY_PATH = "densified-biomass/capacity-by-region/data"


@task(name="fetch-eia-biomass-capacity")
async def fetch_eia_biomass_capacity(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_BIOMASS_CAPACITY_COLUMNS,
                "start": start_year, "end": end_year,
            }
            resp = await manager.get_route(EIA_BIOMASS_CAPACITY_PATH, **params)
...
@task(name="upsert-biomass-capacity")
def upsert_biomass_capacity_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_biomass_capacity(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-biomass-capacity", retries=2)
async def ingest_eia_biomass_capacity(
    date_start: str | None = None, date_end: str | None = None,
) -> int:
    """Fetch EIA biomass capacity and upsert into ingest.eia_biomass_capacity."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_biomass_capacity(
    ...
    total = upsert_biomass_capacity_task(settings.effective_ingest_url, data)
