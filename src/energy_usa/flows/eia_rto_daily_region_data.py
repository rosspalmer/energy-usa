"""Prefect flow: fetch EIA electricity/rto/daily-region-data and upsert.

Daily demand/generation per RTO. Period stored as DATE.
"""
...
async def fetch_eia_rto_daily_region_data(
    *,
    base_url: str,
    api_key: str,
    timeout: float,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_RTO_DAILY_REGION_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_electricity(subpath="rto/daily-region-data/data", **params)
...
@task(name="upsert-rto-daily-region-data")
def upsert_rto_daily_region_data_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_rto_daily_region_data(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-rto-daily-region-data", retries=2)
async def ingest_eia_rto_daily_region_data(
    date_start: str | None = None,
    date_end: str | None = None,
) -> int:
    """Fetch EIA RTO daily region data and upsert into ingest.eia_rto_daily_region_data."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_rto_daily_region_data(
    ...
    total = upsert_rto_daily_region_data_task(settings.effective_ingest_url, data)
