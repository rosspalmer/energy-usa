"""Prefect flow: fetch EIA electricity/rto/region-sub-ba-data and upsert.

Hourly load per sub-balancing authority. Period stored as TEXT.
"""
...
EIA_RTO_SUB_BA_DATA_COLUMNS = ["value", "value-units"]


@task(name="fetch-eia-rto-region-sub-ba-data")
async def fetch_eia_rto_region_sub_ba_data(
    *,
    base_url: str,
    api_key: str,
    timeout: float,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_RTO_SUB_BA_DATA_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_electricity(subpath="rto/region-sub-ba-data/data", **params)
...
@task(name="upsert-rto-region-sub-ba-data")
def upsert_rto_region_sub_ba_data_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_rto_region_sub_ba_data(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-rto-region-sub-ba-data", retries=2)
async def ingest_eia_rto_region_sub_ba_data(
    date_start: str | None = None,
    date_end: str | None = None,
) -> int:
    """Fetch EIA RTO sub-BA hourly data and upsert into ingest.eia_rto_region_sub_ba_data."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_rto_region_sub_ba_data(
    ...
    total = upsert_rto_region_sub_ba_data_task(settings.effective_ingest_url, data)
