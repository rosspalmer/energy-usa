"""Prefect flow: fetch EIA nuclear-outages/facility-nuclear-outages and upsert.

Daily outages per nuclear facility and unit. Period stored as DATE.
"""
...
EIA_NUCLEAR_OUTAGES_FAC_COLUMNS = ["capacity-mw", "outage-mw"]
EIA_NUCLEAR_OUTAGES_FAC_PATH = "nuclear-outages/facility-nuclear-outages/data"


@task(name="fetch-eia-nuclear-outages-facility")
async def fetch_eia_nuclear_outages_facility(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_NUCLEAR_OUTAGES_FAC_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_NUCLEAR_OUTAGES_FAC_PATH, **params)
...
@task(name="upsert-nuclear-outages-facility")
def upsert_nuclear_outages_facility_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_nuclear_outages_facility(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-nuclear-outages-facility", retries=2)
async def ingest_eia_nuclear_outages_facility(
    date_start: str | None = None, date_end: str | None = None,
) -> int:
    """Fetch EIA facility nuclear outages and upsert into ingest.eia_nuclear_outages_facility."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_nuclear_outages_facility(
    ...
    total = upsert_nuclear_outages_facility_task(settings.effective_ingest_url, data)
