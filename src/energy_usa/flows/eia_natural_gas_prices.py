"""Prefect flow: fetch EIA natural-gas/pri/sum and upsert into ingest.eia_natural_gas_prices.

Monthly natural gas prices by area and series. Period stored as DATE (first of month).
"""
...
EIA_NG_PRICES_COLUMNS = ["value"]
EIA_NG_PRICES_PATH = "natural-gas/pri/sum/data"


@task(name="fetch-eia-natural-gas-prices")
async def fetch_eia_natural_gas_prices(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_NG_PRICES_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_NG_PRICES_PATH, **params)
...
@task(name="upsert-natural-gas-prices")
def upsert_natural_gas_prices_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_natural_gas_prices(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-natural-gas-prices", retries=2)
async def ingest_eia_natural_gas_prices(
    date_start: str | None = None, date_end: str | None = None,
) -> int:
    """Fetch EIA natural gas prices and upsert into ingest.eia_natural_gas_prices."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_natural_gas_prices(
    ...
    total = upsert_natural_gas_prices_task(settings.effective_ingest_url, data)
    if total == 0:
