...
pass date_start/date_end for backfill. Requires EIA_API_KEY and INGEST_DATABASE_URL.
"""
...
async def ingest_eia_state_source_disposition(
    ...
    if not settings.ingest_database_url:
        raise ValueError("INGEST_DATABASE_URL is required for ingest_eia_state_source_disposition")
    ...
    total = upsert_state_source_disposition_task(settings.ingest_database_url, data)
    if total == 0:
        logger.warning(
