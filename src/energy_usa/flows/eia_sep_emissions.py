"""Prefect flow: fetch EIA state-electricity-profiles/emissions-by-state-by-fuel and upsert.

Annual CO2/SO2/NOx emissions per state, sector, and fuel. Period stored as DATE (Jan 1).
"""
...
async def fetch_eia_sep_emissions(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_SEP_EMISSIONS_COLUMNS,
                "frequency": "annual", "start": start_year, "end": end_year,
            }
            resp = await manager.get_electricity(
                subpath="state-electricity-profiles/emissions-by-state-by-fuel/data", **params)
...
@task(name="upsert-sep-emissions")
def upsert_sep_emissions_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_sep_emissions(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-sep-emissions", retries=2)
async def ingest_eia_sep_emissions(
    date_start: str | None = None, date_end: str | None = None,
) -> int:
    """Fetch EIA state electricity profile emissions and upsert into ingest.eia_sep_emissions."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_sep_emissions(
    ...
    total = upsert_sep_emissions_task(settings.effective_ingest_url, data)
