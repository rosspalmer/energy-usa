...
When date_start or date_end are omitted, they default to last calendar month.
"""
...
def resolve_date_range(
    ...
    When both are None, returns last calendar month (start and end set to that month).
    When one is None, the other is used and the missing bound defaults to last month's
    start or end respectively.

    :param date_start: Optional start period (YYYY-MM). If None, uses first month of last calendar month.
    :param date_end: Optional end period (YYYY-MM). If None, uses last calendar month.
    ...
    default_end = default_start  # single month for default range
