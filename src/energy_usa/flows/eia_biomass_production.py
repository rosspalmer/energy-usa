"""Prefect flow: fetch EIA densified-biomass/production-by-region and upsert.

Annual wood pellet production by region. Period stored as DATE (Jan 1).
"""
...
EIA_BIOMASS_PRODUCTION_COLUMNS = ["production"]
EIA_BIOMASS_PRODUCTION_PATH = "densified-biomass/production-by-region/data"


@task(name="fetch-eia-biomass-production")
async def fetch_eia_biomass_production(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_BIOMASS_PRODUCTION_COLUMNS,
                "start": start_year, "end": end_year,
            }
            resp = await manager.get_route(EIA_BIOMASS_PRODUCTION_PATH, **params)
...
@task(name="upsert-biomass-production")
def upsert_biomass_production_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_biomass_production(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-biomass-production", retries=2)
async def ingest_eia_biomass_production(
    ...
    data = await fetch_eia_biomass_production(
    ...
    total = upsert_biomass_production_task(settings.effective_ingest_url, data)
