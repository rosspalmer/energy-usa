...
The parent flow splits a provided date range (YYYY-MM to YYYY-MM) into chunks
of N months and submits one child ingest flow run per chunk.
"""
...
from energy_usa.flows.date_range import monthly_chunks
...
async def backfill_eia(
    ...
    chunks = list(monthly_chunks(date_start, date_end, chunk_months=chunk_months))
