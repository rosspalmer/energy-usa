...
EIA_NG_PRODUCTION_COLUMNS = ["value"]
EIA_NG_PRODUCTION_PATH = "natural-gas/prod/sum/data"


@task(name="fetch-eia-natural-gas-production")
async def fetch_eia_natural_gas_production(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_NG_PRODUCTION_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_NG_PRODUCTION_PATH, **params)
...
@task(name="upsert-natural-gas-production")
def upsert_natural_gas_production_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_natural_gas_production(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-natural-gas-production", retries=2)
async def ingest_eia_natural_gas_production(
    ...
    data = await fetch_eia_natural_gas_production(
    ...
    total = upsert_natural_gas_production_task(settings.effective_ingest_url, data)
    if total == 0:
