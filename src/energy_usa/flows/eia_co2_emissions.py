"""Prefect flow: fetch EIA co2-emissions/co2-emissions-aggregates and upsert.

Annual CO2 emissions by state, sector, and fuel. Period stored as DATE (Jan 1).
"""
...
EIA_CO2_COLUMNS = ["value"]
EIA_CO2_PATH = "co2-emissions/co2-emissions-aggregates/data"


@task(name="fetch-eia-co2-emissions")
async def fetch_eia_co2_emissions(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_CO2_COLUMNS,
                "frequency": "annual", "start": start_year, "end": end_year,
            }
            resp = await manager.get_route(EIA_CO2_PATH, **params)
...
@task(name="upsert-co2-emissions")
def upsert_co2_emissions_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_co2_emissions(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-co2-emissions", retries=2)
async def ingest_eia_co2_emissions(
    ...
    data = await fetch_eia_co2_emissions(
    ...
    total = upsert_co2_emissions_task(settings.effective_ingest_url, data)
    if total == 0:
