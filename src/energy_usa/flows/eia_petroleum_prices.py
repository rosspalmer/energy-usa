...
EIA_PETROLEUM_PRICES_COLUMNS = ["value"]
EIA_PETROLEUM_PRICES_PATH = "petroleum/pri/gnd/data"


@task(name="fetch-eia-petroleum-prices")
async def fetch_eia_petroleum_prices(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_PETROLEUM_PRICES_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_PETROLEUM_PRICES_PATH, **params)
...
@task(name="upsert-petroleum-prices")
def upsert_petroleum_prices_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_petroleum_prices(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-petroleum-prices", retries=2)
async def ingest_eia_petroleum_prices(
    ...
    data = await fetch_eia_petroleum_prices(
    ...
    total = upsert_petroleum_prices_task(settings.effective_ingest_url, data)
    if total == 0:
