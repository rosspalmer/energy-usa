"""Prefect flow: fetch EIA coal/aggregate-production and upsert.

Quarterly US coal production by rank and location. Period stored as TEXT.
"""
...
EIA_COAL_AGG_PROD_COLUMNS = ["production"]
EIA_COAL_AGG_PROD_PATH = "coal/aggregate-production/data"


@task(name="fetch-eia-coal-aggregate-production")
async def fetch_eia_coal_aggregate_production(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_COAL_AGG_PROD_COLUMNS,
                "start": start[:4], "end": end[:4],
            }
            resp = await manager.get_route(EIA_COAL_AGG_PROD_PATH, **params)
...
@task(name="upsert-coal-aggregate-production")
def upsert_coal_aggregate_production_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_coal_aggregate_production(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-coal-aggregate-production", retries=2)
async def ingest_eia_coal_aggregate_production(
    ...
    data = await fetch_eia_coal_aggregate_production(
    ...
    total = upsert_coal_aggregate_production_task(settings.effective_ingest_url, data)
