...
EIA_CRUDE_OIL_IMPORTS_COLUMNS = ["quantity"]
EIA_CRUDE_OIL_IMPORTS_PATH = "crude-oil-imports/data"


@task(name="fetch-eia-crude-oil-imports")
async def fetch_eia_crude_oil_imports(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_CRUDE_OIL_IMPORTS_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_CRUDE_OIL_IMPORTS_PATH, **params)
...
@task(name="upsert-crude-oil-imports")
def upsert_crude_oil_imports_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_crude_oil_imports(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-crude-oil-imports", retries=2)
async def ingest_eia_crude_oil_imports(
    ...
    data = await fetch_eia_crude_oil_imports(
    ...
    total = upsert_crude_oil_imports_task(settings.effective_ingest_url, data)
