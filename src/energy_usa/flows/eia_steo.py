...
EIA_STEO_COLUMNS = ["value"]
EIA_STEO_PATH = "steo/data"


@task(name="fetch-eia-steo")
async def fetch_eia_steo(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_STEO_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_STEO_PATH, **params)
...
@task(name="upsert-steo")
def upsert_steo_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_steo(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-steo", retries=2)
async def ingest_eia_steo(
    ...
    total = upsert_steo_task(settings.effective_ingest_url, data)
    if total == 0:
