...
EIA_TOTAL_ENERGY_COLUMNS = ["value"]
EIA_TOTAL_ENERGY_PATH = "total-energy/data"


@task(name="fetch-eia-total-energy")
async def fetch_eia_total_energy(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_TOTAL_ENERGY_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_TOTAL_ENERGY_PATH, **params)
...
@task(name="upsert-total-energy")
def upsert_total_energy_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_total_energy(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-total-energy", retries=2)
async def ingest_eia_total_energy(
    ...
    data = await fetch_eia_total_energy(
    ...
    total = upsert_total_energy_task(settings.effective_ingest_url, data)
