"""Prefect flow: fetch EIA natural-gas/cons/sum and upsert into ingest.eia_natural_gas_consumption.

Monthly natural gas consumption by area and sector. Period stored as DATE (first of month).
"""
...
EIA_NG_CONSUMPTION_COLUMNS = ["value"]
EIA_NG_CONSUMPTION_PATH = "natural-gas/cons/sum/data"


@task(name="fetch-eia-natural-gas-consumption")
async def fetch_eia_natural_gas_consumption(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_NG_CONSUMPTION_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_route(EIA_NG_CONSUMPTION_PATH, **params)
...
@task(name="upsert-natural-gas-consumption")
def upsert_natural_gas_consumption_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_natural_gas_consumption(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-natural-gas-consumption", retries=2)
async def ingest_eia_natural_gas_consumption(
    date_start: str | None = None, date_end: str | None = None,
) -> int:
    """Fetch EIA natural gas consumption and upsert into ingest.eia_natural_gas_consumption."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_natural_gas_consumption(
    ...
    total = upsert_natural_gas_consumption_task(settings.effective_ingest_url, data)
