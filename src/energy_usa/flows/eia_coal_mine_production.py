"""Prefect flow: fetch EIA coal/mine-production and upsert.

Quarterly coal production by state, mine type, and rank. Period stored as TEXT.
"""
...
EIA_COAL_MINE_PROD_COLUMNS = ["production"]
EIA_COAL_MINE_PROD_PATH = "coal/mine-production/data"


@task(name="fetch-eia-coal-mine-production")
async def fetch_eia_coal_mine_production(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_COAL_MINE_PROD_COLUMNS,
                "start": start[:4], "end": end[:4],
            }
            resp = await manager.get_route(EIA_COAL_MINE_PROD_PATH, **params)
...
@task(name="upsert-coal-mine-production")
def upsert_coal_mine_production_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_coal_mine_production(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-coal-mine-production", retries=2)
async def ingest_eia_coal_mine_production(
    date_start: str | None = None, date_end: str | None = None,
) -> int:
    """Fetch EIA coal mine production and upsert into ingest.eia_coal_mine_production."""
    logger = get_run_logger()
    settings = Settings()
    ...
    data = await fetch_eia_coal_mine_production(
    ...
    total = upsert_coal_mine_production_task(settings.effective_ingest_url, data)
    if total == 0:
