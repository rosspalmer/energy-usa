...
EIA_RTO_REGION_DATA_COLUMNS = ["value", "value-units"]


@task(name="fetch-eia-rto-region-data")
async def fetch_eia_rto_region_data(
    *,
    base_url: str,
    api_key: str,
    timeout: float,
    ...
    """Fetch EIA RTO region data for the given date range.

    :param start: Start date (YYYY-MM or YYYY-MM-DD).
    :param end: End date (YYYY-MM or YYYY-MM-DD).
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "offset": offset,
                "data[]": EIA_RTO_REGION_DATA_COLUMNS,
                "start": start,
                "end": end,
            }
            resp = await manager.get_electricity(subpath="rto/region-data/data", **params)
...
@task(name="upsert-rto-region-data")
def upsert_rto_region_data_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_rto_region_data(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-rto-region-data", retries=2)
async def ingest_eia_rto_region_data(
    ...
    :param date_end: End (YYYY-MM). Defaults to last month.
    ...
    data = await fetch_eia_rto_region_data(
    ...
    total = upsert_rto_region_data_task(settings.effective_ingest_url, data)
