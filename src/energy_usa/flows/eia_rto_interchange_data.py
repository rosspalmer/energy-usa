"""Prefect flow: fetch EIA electricity/rto/interchange-data and upsert.

Hourly interchange (power flows) between RTOs. Period stored as TEXT.
"""
...
EIA_RTO_INTERCHANGE_COLUMNS = ["value"]


@task(name="fetch-eia-rto-interchange-data")
async def fetch_eia_rto_interchange_data(
    *,
    base_url: str,
    api_key: str,
    timeout: float,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_RTO_INTERCHANGE_COLUMNS,
                "start": start, "end": end,
            }
            resp = await manager.get_electricity(subpath="rto/interchange-data/data", **params)
...
@task(name="upsert-rto-interchange-data")
def upsert_rto_interchange_data_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_rto_interchange_data(conn, rows)
    finally:
        conn.close()
...
@flow(
    name="ingest-eia-rto-interchange-data",
    flow_run_name=_run_name,
    retries=2,
    retry_delay_seconds=60,
    timeout_seconds=900,
)
async def ingest_eia_rto_interchange_data(
    ...
    data = await fetch_eia_rto_interchange_data(
    ...
    total = upsert_rto_interchange_data_task(settings.ingest_database_url, data)
    if total == 0:
