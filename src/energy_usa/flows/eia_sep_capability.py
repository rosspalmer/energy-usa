"""Prefect flow: fetch EIA state-electricity-profiles/capability and upsert.

Annual generating capability per state and fuel type. Period stored as DATE (Jan 1).
"""
...
EIA_SEP_CAPABILITY_COLUMNS = ["nameplate-capacity-mw", "net-summer-capacity-mw", "net-winter-capacity-mw"]


@task(name="fetch-eia-sep-capability")
async def fetch_eia_sep_capability(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_SEP_CAPABILITY_COLUMNS,
                "frequency": "annual", "start": start_year, "end": end_year,
            }
            resp = await manager.get_electricity(
                subpath="state-electricity-profiles/capability/data", **params)
...
@task(name="upsert-sep-capability")
def upsert_sep_capability_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_sep_capability(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-sep-capability", retries=2)
async def ingest_eia_sep_capability(
    ...
    data = await fetch_eia_sep_capability(
    ...
    total = upsert_sep_capability_task(settings.effective_ingest_url, data)
