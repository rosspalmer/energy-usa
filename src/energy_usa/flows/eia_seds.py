...
EIA_SEDS_COLUMNS = ["value"]
EIA_SEDS_PATH = "seds/data"


@task(name="fetch-eia-seds")
async def fetch_eia_seds(
    *,
    ...
    try:
        ...
        while True:
            params: dict[str, Any] = {
                ...
                "data[]": EIA_SEDS_COLUMNS,
                "frequency": "annual", "start": start_year, "end": end_year,
            }
            resp = await manager.get_route(EIA_SEDS_PATH, **params)
...
@task(name="upsert-seds")
def upsert_seds_task(database_url: str, rows: list[dict[str, Any]]) -> int:
    conn = get_connection(database_url)
    try:
        return upsert_seds(conn, rows)
    finally:
        conn.close()


@flow(name="ingest-eia-seds", retries=2)
async def ingest_eia_seds(
    ...
    total = upsert_seds_task(settings.effective_ingest_url, data)
    if total == 0:
