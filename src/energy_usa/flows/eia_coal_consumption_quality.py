...
@flow(name="ingest-eia-coal-consumption-quality", retries=2)
async def ingest_eia_coal_consumption_quality(
    date_start: str | None = None, date_end: str | None = None,
) -> int:
    """Fetch EIA coal consumption/quality and upsert into ingest.eia_coal_consumption_quality."""
    logger = get_run_logger()
    settings = Settings()
    ...
    total = upsert_coal_consumption_quality_task(settings.effective_ingest_url, data)
