"""Upsert EIA natural-gas/prod/sum into ingest.eia_natural_gas_production.

Monthly natural gas production by area and series.
...
def upsert_natural_gas_production(conn: psycopg.Connection, rows: list[dict[str, Any]]) -> int:
    """Upsert EIA natural gas production rows.

    :param conn: Open psycopg connection.
    :param rows: List of dicts with EIA natural-gas/prod/sum keys.
    :returns: Number of rows upserted.
    """
    if not rows:
        return 0
    sql = """
    INSERT INTO ingest.eia_natural_gas_production
