"""Upsert EIA natural-gas/stor/sum into ingest.eia_natural_gas_storage.

Monthly natural gas underground storage by area and series.
...
def upsert_natural_gas_storage(conn: psycopg.Connection, rows: list[dict[str, Any]]) -> int:
    """Upsert EIA natural gas storage rows.

    :param conn: Open psycopg connection.
    :param rows: List of dicts with EIA natural-gas/stor/sum keys.
    :returns: Number of rows upserted.
    """
    if not rows:
        return 0
    sql = """
    INSERT INTO ingest.eia_natural_gas_storage
