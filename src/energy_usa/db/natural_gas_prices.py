"""Upsert EIA natural-gas/pri/sum into ingest.eia_natural_gas_prices.

Monthly natural gas prices by area, process, and series.
...
def upsert_natural_gas_prices(conn: psycopg.Connection, rows: list[dict[str, Any]]) -> int:
    """Upsert EIA natural gas price rows.

    :param conn: Open psycopg connection.
    :param rows: List of dicts with EIA natural-gas/pri/sum keys.
    :returns: Number of rows upserted.
    """
    if not rows:
        return 0
    sql = """
    INSERT INTO ingest.eia_natural_gas_prices
