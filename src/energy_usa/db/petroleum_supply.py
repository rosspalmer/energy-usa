"""Upsert EIA petroleum/sum/snd into ingest.eia_petroleum_supply.

Monthly petroleum supply and disposition by area, product, and process.
...
def upsert_petroleum_supply(conn: psycopg.Connection, rows: list[dict[str, Any]]) -> int:
    """Upsert EIA petroleum supply rows.

    :param conn: Open psycopg connection.
    :param rows: List of dicts with EIA petroleum/sum/snd keys.
    :returns: Number of rows upserted.
    """
    if not rows:
        return 0
    sql = """
    INSERT INTO ingest.eia_petroleum_supply
